...
describe('PrismaWebinarRepository', () => {
  ...
  beforeAll(async () => {
    ...
    describe('Scenario : findById', () => {
        testCreateWebinar();
        it('should find the created webinar by ID', async () => {
            // ARRANGE
            const webinarId = 'webinar-id'
            // ACT
            const foundWebinar = await repository.findById(webinarId)
            // ASSERT
    ...
    describe('Scenario : update', () => {
        testCreateWebinar();
        it('should update the created webinar with needed data', async () => {
            // ARRANGE
            const newWebinar = new Webinar({
                id: 'webinar-id',
                organizerId: 'organizer-id',
                title: 'Webinar title B',
                startDate: new Date('2022-01-01T00:00:00Z'),
                endDate: new Date('2022-01-01T01:00:00Z'),
                seats: 100,
            });
